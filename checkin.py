...
-----------------------------------------------------------------------------
 ...
 【2026-09-11 验证通过】run #20 从 Azure runner 成功领取
   仅把 TRAE_DEVICE_ID 换成注册设备号（20…46(16位)）后，推送触发的 run #20
   即走 claim 分支成功（11 秒）。此前 6 次全 9074 的真正原因是**设备号填错**
   （填了脚本自造号），而不是机房 IP —— 「IP 限流」的旧结论已作废。
